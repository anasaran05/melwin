...
export interface SendAuthConfirmationEmailParams {
  to: string
  founderName?: string
  confirmUrl: string
}
...
export async function sendAuthConfirmationEmail({
  ...
  founderName,
  confirmUrl,
}: SendAuthConfirmationEmailParams): Promise<{ success: boolean; id?: string; error?: string }> {
  try {
    const resend = getResendClient()
    const displayName = founderName?.trim() || 'Founder'
      ...
      <html>
        <head>
          ...
          <style>
            ...
            p { font-size: 14px; line-height: 1.6; color: #a1a1aa; margin: 0 0 20px 0; }
            .highlight-box { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 20px; margin: 24px 0; }
            .btn { display: inline-block; background: #38bdf8; color: #000000; font-weight: 800; font-size: 13px; text-decoration: none; padding: 14px 30px; border-radius: 100px; margin-top: 12px; }
            .footer { font-size: 11px; color: #71717a; text-align: center; margin-top: 32px; border-top: 1px solid rgba(255,255,255,0.08); padding-top: 20px; }
          </style>
        </head>
        <body>
          <div class="card">
            <span class="badge">BMF Founders Club &bull; Account Verification</span>
            <h1>Confirm Your Email, ${displayName} 👋</h1>
            <p>Welcome to the <strong>BMF Founders Club</strong>. Please click the button below to verify your email address and activate your founder pass.</p>
            
            <div style="text-align: center; margin: 28px 0;">
              <a href="${confirmUrl}" class="btn">Confirm Email Address &rarr;</a>
            </div>

            <div class="highlight-box">
              <p style="margin: 0; color: #ffffff; font-weight: 600; font-size: 13px;">🔒 Security Notice</p>
              <p style="margin: 6px 0 0 0; font-size: 12px; color: #71717a; line-height: 1.5;">
                This verification link will expire in 24 hours. If you did not create a BMF Founders account with this email address, you can safely disregard this message.
              </p>
            </div>
    ...
    if (!resend) {
      console.log(`[Resend Mock Email Dispatch to ${to}]: Account Confirmation for ${displayName}`)
    ...
    const { data, error } = await resend.emails.send({
      from: FROM_EMAIL,
      to,
      subject: `✨ Confirm your BMF Club email address (${displayName})`,
      html,
    })
...
export interface SendAuthMagicLinkEmailParams {
  to: string
  loginUrl: string
}
...
export async function sendAuthMagicLinkEmail({
  to,
  loginUrl,
}: SendAuthMagicLinkEmailParams): Promise<{ success: boolean; id?: string; error?: string }> {
  try {
    const resend = getResendClient()
      ...
      <html>
        <head>
          ...
          <style>
            ...
            p { font-size: 14px; line-height: 1.6; color: #a1a1aa; margin: 0 0 20px 0; }
            .highlight-box { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 20px; margin: 24px 0; }
            .btn { display: inline-block; background: #ffffff; color: #000000; font-weight: 800; font-size: 13px; text-decoration: none; padding: 14px 30px; border-radius: 100px; margin-top: 12px; }
            .footer { font-size: 11px; color: #71717a; text-align: center; margin-top: 32px; border-top: 1px solid rgba(255,255,255,0.08); padding-top: 20px; }
          </style>
        </head>
        <body>
          <div class="card">
            ...
            <div class="highlight-box">
              <p style="margin: 0; color: #ffffff; font-weight: 600; font-size: 13px;">⏱️ Link Expiration</p>
              <p style="margin: 6px 0 0 0; font-size: 12px; color: #71717a; line-height: 1.5;">
    ...
    if (!resend) {
      console.log(`[Resend Mock Email Dispatch to ${to}]: Magic Link`)
      return { success: true, id: 'mock-magic-link-id' }
    }

    const { data, error } = await resend.emails.send({
      from: FROM_EMAIL,
      to,
      subject: `🔑 Sign in to BMF Founders Club`,
      html,
    })
